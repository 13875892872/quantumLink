<template>
  <login-layout>
    <LoginContainer :subTitle="$t('views.system.theme.defaultSlogan')">
      <h2 class="mb-24">{{ $t('views.customerLogin.userRegister') }}</h2>
      ...
      <el-button size="large" type="primary" class="w-full" @click="register">{{
        $t('views.customerLogin.buttons.register')
      }}</el-button>
...
const rules = ref<FormRules<RegisterRequest>>({
  username: [
    {
      required: true,
      message: t('views.customerLogin.userForm.form.username.placeholder'),
      trigger: 'blur'
    },
    {
      min: 6,
      max: 20,
      message: t('views.customerLogin.userForm.form.username.lengthMessage'),
      trigger: 'blur'
    }
  ],
  password: [
    {
      required: true,
      message: t('views.customerLogin.userForm.form.password.requiredMessage'),
      trigger: 'blur'
    },
    {
      min: 6,
      max: 20,
      message: t('views.customerLogin.userForm.form.password.lengthMessage'),
      trigger: 'blur'
    }
  ],
  re_password: [
    {
      required: true,
      message: t('views.customerLogin.userForm.form.re_password.requiredMessage'),
      trigger: 'blur'
    },
    {
      min: 6,
    ...
    },
    {
      validator: (rule, value, callback) => {
        if (registerForm.value.password != registerForm.value.re_password) {
          callback(new Error(t('views.customerLogin.userForm.form.password.validatorMessage')))
        } else {
          callback()
        }
      },
  ...
  email: [
    {
      required: true,
      message: t('views.customerLogin.userForm.form.email.requiredMessage'),
      trigger: 'blur'
    },
    {
      validator: (rule, value, callback) => {
        ...
        if (!emailRegExp.test(value) && value != '') {
          callback(new Error(t('views.customerLogin.userForm.form.email.validatorEmail')))
        } else {
          callback()
        }
      },
  ...
  code: [{ required: true, message: t('views.customerLogin.verificationCode.placeholder') }]
})
